...
uint256 hashGenesisBlock("0x38e4ee29df82718fe31c0fcdf7d18ad6232b1fb985dd6a8a8f79a6a2bc891d8c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Goodcoin: starting difficulty is 1 / 2^12
...
{
    // Goodcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000000); // Goodcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Goodcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Goodcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Goodcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Goodcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa10a245e27c4373928c867e4ec5852cf84a250e26cf673905ce834c6885acfdf");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "es qez sirum em";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f943630023114c85b602c8d12400b26a481942015305ba4ff09b4351a41a94138cee53f4261507e29cadacc232d11869a11f8610665e209bddf200272a1535e8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1597253761;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084861981;

        if (fTestNet)
        {
            block.nTime    = 1597253750;
            block.nNonce   = 385649036;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf49a270e192f4f4a454cea51d4b0f9747b65c390f63078909504d0921abfd68a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Goodcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GoodcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GoodcoinMiner(CWallet *pwallet)
{
    printf("GoodcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running GoodcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GoodcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GoodcoinMiner, pwallet));
}
